...
Namespace NonPersistentObjectsDemo.Module.BusinessObjects
        ...
        Public ReadOnly Property Orders As IList(Of Order)
            Get
                If _Orders Is Nothing AndAlso ObjectSpaceProp IsNot Nothing Then
                    _Orders = ObjectSpaceProp.GetObjects(Of Order)(Criteria).ToArray()
                End If
    ...
    Friend Module LiveSummaryPresetStorage
        ...
        Sub New()
            ...
            Call presets.Add(Guid.NewGuid(), New LiveSummaryPreset() With {.Name = "Tentative", .Status = OrderStatus.Pending, .Period = 1})
            Call presets.Add(Guid.NewGuid(), New LiveSummaryPreset() With {.Name = "To produce", .Status = OrderStatus.Confirmed, .Period = 100})
            Call presets.Add(Guid.NewGuid(), New LiveSummaryPreset() With {.Name = "To deliver", .Status = OrderStatus.Ready, .Period = 100})
            Call presets.Add(Guid.NewGuid(), New LiveSummaryPreset() With {.Name = "Canceled this week", .Status = OrderStatus.Canceled, .Period = 7})
            Call presets.Add(Guid.NewGuid(), New LiveSummaryPreset() With {.Name = "Delivered this week", .Status = OrderStatus.Delivered, .Period = 7})
        End Sub
